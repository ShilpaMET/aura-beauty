        ...
        blok.filters?.forEach((f: any) => {
            if (f.filter_type === 'range') {
                defaults[f.group_name] = {
                    min: f.min_price ?? 0,
                    max: f.max_price ?? Infinity, // ✅ allow all prices initially
                };
            } else if (f.filter_type === 'checkbox') {
                defaults[f.group_name] = []; // ✅ empty means no filter
            } else if (f.filter_type === 'switch') {
                defaults[f.group_name] = null; // ✅ ignore switch until toggled
            }
        });
    ...
    const filteredProducts = useMemo(() => {
        return products.filter((product) => {
            ...
            for (const [group, value] of Object.entries(selectedFilters)) {
                ...
                if (typeof value === 'object' && value.min === 0 && value.max === Infinity) continue;

                if (Array.isArray(value)) {
                    const field = group.toLowerCase();
                    if (!value.some((v) => content[field]?.includes(v) || content[field] === v)) {
                        return false;
                    }
                }
    ...
    return (
        <section {...storyblokEditable(blok)} className="bg-[#F8F8F8] min-h-screen py-10">
            ...
            <div className="max-w-7xl mx-auto flex gap-8 px-4 flex-wrap">
                ...
                <div className={`grid gap-6 flex-1 sm:grid-cols-2 md:grid-cols-3 lg:${filterOpen ? 'grid-cols-3' : 'grid-cols-4'} xl:${filterOpen ? 'grid-cols-3' : 'grid-cols-5'}`}>
                    {loading ? (
